...
export function SlideThumbnail({
  title, index, thumbnail, backgroundColor,
  isActive, onClick, onDuplicate, onDelete, onMoveUp, onMoveDown,
  isFirst, isLast,
}: ThumbnailProps) {
  return (
    <div
    ...
    >
      ...
      <div className={`flex items-center gap-1.5 px-2 py-1.5 text-xs
        ...
        <span className="text-textMuted font-mono w-4 text-center shrink-0">{index + 1}</span>
      ...
      <div className="absolute top-1 right-1 hidden group-hover:flex flex-col gap-0.5">
        {!isFirst && (
          <button onClick={(e) => { e.stopPropagation(); onMoveUp() }}
            className="p-0.5 bg-panel/90 rounded text-textSecondary hover:text-textPrimary text-[10px]">↑</button>
        )}
        {!isLast && (
        ...
        <button onClick={(e) => { e.stopPropagation(); onDuplicate() }}
          className="p-0.5 bg-panel/90 rounded text-textSecondary hover:text-accent text-[10px]" title="Duplicate">⧉</button>
        <button onClick={(e) => { e.stopPropagation(); onDelete() }}
          className="p-0.5 bg-panel/90 rounded text-textSecondary hover:text-danger text-[10px]" title="Delete">✕</button>
      </div>
    </div>
  )
}
