...
const getApi = (canvas: fabric.Canvas) => canvas as fabric.Canvas & {
  hpmUndo?: () => void
  hpmRedo?: () => void
}

export function selectAllObjects(canvas: fabric.Canvas) {
  const objects = canvas.getObjects().filter((obj) => (obj as any).customType !== 'shape-guide')
...
export function deleteActiveObject(canvas: fabric.Canvas) {
  ...
  if (active.type === 'activeSelection') {
    ;(active as fabric.ActiveSelection).getObjects().forEach((obj) => canvas.remove(obj))
  } else {
...
export function duplicateActiveObject(canvas: fabric.Canvas) {
  ...
  active.clone((cloned: any) => {
    if (active.type === 'activeSelection' && cloned.type === 'activeSelection') {
      cloned.canvas = canvas
      cloned.getObjects().forEach((obj: any) => {
        obj.set({ left: (obj.left ?? 0) + 20, top: (obj.top ?? 0) + 20 })
        canvas.add(obj)
      })
      canvas.setActiveObject(new fabric.ActiveSelection(cloned.getObjects(), { canvas }))
    } else {
