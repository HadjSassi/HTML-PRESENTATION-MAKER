...
interface Store {
  ...
  setPresentationInfo: (
    info: Partial<Pick<Presentation, "title" | "author" | "savePath">>,
  ) => void;
  addSlide: () => void;
...
const renumberSlides = (slides: Slide[]) => {
  return slides.map((slide, index) => ({
    ...slide,
    title: slide.title.match(/^Slide \d+$/)
      ? `Slide ${index + 1}`
      : slide.title,
  }));
};
...
export const usePresentationStore = create<Store>()(
  immer((set) => ({
    ...
    reset: () =>
      set((s) => {
        s.presentation = newPresentation();
        s.currentSlideIndex = 0;
        s.selectedObjectId = null;
        s.lastSelectedTextColor = null;
        s.shapeStyle = defaultShapeStyle;
        s.shapeDrawMode = "none";
        s.isDirty = false;
      }),
    load: (p) =>
      set((s) => {
    ...
    setPresentationInfo: (info) =>
      set((s) => {
        Object.assign(s.presentation, info);
        s.isDirty = true;
      }),
    ...
    updateSlideBackground: (idx, color) =>
      set((s) => {
        s.presentation.slides[idx].backgroundColor = color;
        s.isDirty = true;
      }),
    updateSlideAnimation: (idx, anim) =>
      set((s) => {
        s.presentation.slides[idx].animation = anim;
        s.isDirty = true;
      }),
    updateSlideTitle: (idx, title) =>
      set((s) => {
        s.presentation.slides[idx].title = title;
        s.isDirty = true;
      }),
    ...
    setLastSelectedTextColor: (color) =>
      set((s) => {
        s.lastSelectedTextColor = color;
      }),
    setShapeStyle: (style) =>
      set((s) => {
        s.shapeStyle = { ...s.shapeStyle, ...style };
      }),
    setShapeDrawMode: (mode) =>
      set((s) => {
        s.shapeDrawMode = mode;
      }),
    togglePreview: () =>
      set((s) => {
    ...
    updateObject: (props) =>
      set((s) => {
        ...
        if (obj) {
          ...
          (window as any).__fabric_canvas?.fire("object:modified", {
            target: obj,
          } as any);
        }
      }),
